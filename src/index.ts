...
export function titleCase(str: string) {
  return camelToWords(studlyCase(camelCase(str)))
    .join(" ")
    .replace(/\bid\b/gi, "ID");
}
...
export function* iterateAll(
  val: string | any[] | object | NodeList,
  opt: iterateAll_Options = {}
): Generator<{ value: any; index?: number; key?: string }> {
  // opt: {reverse, exclude}
  if (!opt.reverse) {
    // @ts-ignore
    if (val.length != null) {
...
export function mapObjects(
  arr: any[],
  idKey: string | mapObjects_idKey_function
): object {
  const r = {};
  const len = arr.length;
  for (let i = 0; i < len; i++) {
...
export function watchChange<T>(
  getVal: ReplaceReturnType<OmitFirstArg<T>, any>,
  handler: T
) {
  let oldVal;
  const update = (...args) => {
...
export function findParent(
  el: HTMLElement,
  callback: findParent_Callback,
  opt: { withSelf?: boolean } = {}
) {
...
export function getBorder(el: HTMLElement) {
  ...
  return {
    ...
    top: of.y + 50,
    bottom:
      body.offsetHeight < glb().innerHeight
        ? glb().innerHeight
        : body.offsetHeight,
  };
}

export function setElChildByIndex(
  el: HTMLElement,
  child: HTMLElement,
  index: number
) {
...
export function onDOMMany(
  els: (HTMLElement | Window | Document)[],
  names: string[],
  handler: EventHandler,
  ...args: any[]
) {
...
export function findNodeList(
  list: NodeList,
  callback: findNodeList_Callback,
  opt: { reverse?: boolean } = {}
) {
...
export function findNodeListReverse(
  list: NodeList,
  callback: findNodeList_Callback
) {
...
export function binarySearch(
  arr: any[] | NodeList,
  callback: binarySearch_Callback,
  opt: binarySearch_Options = {}
): binarySearch_Return {
  opt = {
    start: 0,
    end: arr.length - 1,
  ...
  return returnNearestIfNoHit
    ? { index: midNum, value: mid, count: i + 1, hit: false, greater: r > 0 }
    : null;
}
